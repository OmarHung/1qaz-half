...
3. **第一次開啟**：直接雙擊會出現「無法驗證開發者」的警告，請依以下步驟處理：
   1. 前往 **系統設定 → 隱私權與安全性**
   2. 往下捲動，找到「已封鎖 1qaz Half，因為它並非來自已識別的開發者」
   3. 點擊旁邊的 **「仍要開啟」**
   4. 在彈出的確認視窗再次點擊 **「開啟」**

   > 或者在 Finder 中對 app **按右鍵 → 開啟**，可直接略過此警告。
...
- **功能開關**：各功能可單獨開啟或關閉
- **選字狀態清除方式**：在注音組字或選字狀態下觸發功能鍵時，需先清除 IME buffer 才能輸出目標字元。提供兩種實作方式：

  | 模式 | 實作方式 | 優點 | 缺點 |
  |------|---------|------|------|
  | `Enter 提交選字`（預設） | 注入 Enter 鍵讓 IME 自行提交目前選字，再輸出目標字元 | 無延遲、即時 | ⚠️ **副作用**：若錯誤判斷為「有組字」（例如滑鼠點選提交了注音但狀態未同步），Enter 可能直接送至 app，在 Line、Telegram 等聊天軟體中造成文字提早送出 |
  | `切換輸入法取消組字` | 暫時切換至 ABC 輸入法（自動取消組字）→ 輸出字元 → 切回注音 | 完全不送任何鍵盤事件給 app | 約 100ms 延遲，輸入法圖示會短暫閃動 |

  > **原理補充**：macOS 沒有提供公開 API 讓外部程式查詢 IME 目前是否在選字狀態，因此兩種方式都是透過追蹤「是否有注音字母鍵被輸入過」作為啟發式判斷（heuristic）。
- **開機自動啟動**
- **檢查更新**
